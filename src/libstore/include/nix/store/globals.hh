    ...
    Setting<PathSet> sandboxPaths{
        ...
        R"(
          ...
          - `:idmap=IDMAP` will make an ID-mapped mount. For more details see
            ...
            `<type>=<id-from>-<id-to>[-<range>]` where type is either `u` or `g`,
            `id-to` defines the first ID of the range mapped inside the sandbox,
            `id-from` is the first ID outside the sandbox
            and `range` is the size of mapped ID range (1 if unset). Multiple
            mappings can be given by separating them with ",". For example,
            `:idmap=u=30000-1000,g=30000-100-1` will map UID 1000 and GID 100
