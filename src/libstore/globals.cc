...
#ifdef __APPLE__
    sandboxPaths = splitString<Strings>("/System/Library/Frameworks /System/Library/PrivateFrameworks /bin/sh /bin/bash /private/tmp /private/var/tmp /usr/lib", " ");
 ...
 */
SandboxPaths SandboxPaths::parse(const std::string_view & str, const std::string & ctx)
{
    Paths res;
    ...
    if (str.starts_with('{')) {
        for (auto & [k, v] : nlohmann::json::parse(str, nullptr, false, true).template get<std::map<Path, SandboxPath>>())
            add(k, std::move(v));
    } else {
